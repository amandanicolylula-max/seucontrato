...
import { Plus, Search, FileScan, ChevronRight, Calendar } from 'lucide-react'
...
export default function AnaliseCorplawList() {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="font-display text-3xl text-navy-900">Análise Corplaw</h1>
            <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-xs font-semibold uppercase tracking-wider">Beta</span>
          </div>
          <p className="text-slate-500 text-sm mt-1">
            {analyses.length} análise(s) — schema rico com riscos numerados, matriz e HTML interativo
          </p>
        </div>
        <Link to="/painel/analise-corplaw/nova" className="flex items-center gap-2 bg-brand hover:bg-brand-light text-white px-4 py-2.5 rounded-xl text-sm font-medium transition-all shadow-sm">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-100">
          ...
          <p className="text-slate-500 text-sm">Nenhuma análise Corplaw ainda. Comece uma nova.</p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm divide-y divide-slate-100">
          {filtered.map(a => (
            <Link key={a.id} to={`/painel/analise-corplaw/${a.id}`} className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50 transition-colors">
