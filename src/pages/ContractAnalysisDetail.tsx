...
// PDF is generated in a Web Worker to avoid freezing the main thread
import {
  ArrowLeft, Download, Mail, CheckCircle2, Loader2,
...
export default function ContractAnalysisDetail() {
  ...
  useEffect(() => {
    load()
    // Subscrição Realtime enquanto processando
    const channel = supabase
      .channel(`analysis-detail-${id}`)
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'contract_analyses', filter: `id=eq.${id}` },
        () => { load() }
      )
      .subscribe()
  ...
  const handleFinalize = async () => {
    ...
    else { toast.success('Parecer finalizado!'); setAnalysis(prev => prev ? { ...prev, status: 'finalizado' } : prev) }
    setFinalizing(false)
  }
  ...
  const handleExportPDF = () => {
    ...
    // Run PDF generation in a Web Worker to avoid freezing the main thread
      ...
      <div className="max-w-lg mx-auto py-24 flex flex-col items-center text-center gap-6">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            A IA está analisando o contrato. Aguarde alguns instantes.
          </p>
        </div>
      </div>
    )
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {risk && (
                <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${risk.cls}`}>
                  Risco {risk.label}
                </span>
              )}
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {deleteConfirm ? (
            <div className="flex items-center gap-1 bg-red-50 border border-red-200 rounded-xl px-3 py-2">
              <span className="text-xs text-red-700 font-medium mr-1">Excluir análise?</span>
              <button
                onClick={handleDelete}
                disabled={deleting}
                className="text-xs font-semibold text-red-600 hover:text-red-700 px-2 py-0.5 rounded hover:bg-red-100 transition-colors"
              >
          ...
          <a
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 text-sm text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all"
          >
          ...
          {sections && (
            <button
              ...
              className="flex items-center gap-2 bg-brand hover:bg-brand-light text-white px-3 py-2 rounded-xl text-sm font-medium transition-all shadow-sm disabled:opacity-60"
            >
              {exportingPDF
      ...
      {sections?.secoes && (
        <div className="space-y-3">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide px-1">
            Seções do Parecer — edite diretamente abaixo
          </p>
          {sections.secoes.map(s => (
            <SectionEditor key={s.id} section={s} onSave={handleSaveSection} />
          ))}
        </div>
      )}

      {/* Finalizar */}
      {analysis.status === 'rascunho' && (
